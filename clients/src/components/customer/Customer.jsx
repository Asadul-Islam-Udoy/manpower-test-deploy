...
const Customer = () => {
  ...
  return (
    <>
      {lodding && <Lodder />}
      <section ref={ref} className="bg-blue-50 relative mt-10">
        <div className="absolute top-6 left-[8%] md:top-14 md:left-[30%]">
          <motion.h1
            initial={{ opacity: 0, x: -100 }}
            animate={isInView ? { opacity: 1, x: 0 } : {}}
            transition={{ duration: 1.5 }}
            className="text-lg md:text-4xl text-center text-black font-serif"
          >
            {videos[0]?.title.substring(0, 20)}
            <span className="text-[#41B3A2]">
              {" "}
              {videos[0]?.title.substring(20, videos[0].length)}
            </span>
          </motion.h1>
        </div>
        <div class="relative flex flex-col items-center max-w-[90%] sm:pt-20 px-4 mx-auto md:flex-row sm:px-6 p-8">
          <div class="flex items-center pt-16  md:pb-14 md:w-1/2  md:pr-10">
            <motion.div
              ...
              class="text-left"
            >
              <blockquote class="relative border-s-4 ps-4 sm:ps-6 border-[#41B3A2]">
