...
exports.listServices = async (req, res) => {
  try {
    ...
    //   "service_category_id  reviews reviews.user"
    // );
    const ApiFucture = new ServicesSearch(Service.find().populate("service_category_id  reviews reviews.user"),req.query).search();
...
exports.categoryIdServices = async (req, res) => {
  try {
    const services = await Service.find({ service_category_id: req.params.id }).populate('service_category_id reviews reviews.user');
    if (!services) {
      return res.status(400).json({
        flag: false,
...
exports.homePagesNewServiceController = async (req, res) => {
  try {
    ...
    if(services.length > 0){
      servicesList = services.filter((item)=>new Date(item.updatedAt.getTime()+40 * 24 * 60 * 60 * 1000) >= futureDate);
    } 
    res.status(200).json({
      flag: true,
      message: " get home services successfully!",
